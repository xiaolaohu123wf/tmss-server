...
- 大屏前端引入统一日期范围筛选组件（`DateRangePicker`），屏内统计查询参数统一
- 车辆点击高亮轨迹采用“实时在途优先”选段策略，修复历史错段问题
- 设备创建设备采用“同 IMEI 软删恢复优先，活跃冲突返回 409”语义
...
**核心优势**：国内底图能力成熟，覆盖实时监控与轨迹回放场景。  
**坐标约定**：数据库保留设备原始 WGS-84；HTTP 路由层转换为 GCJ-02 后返回前端，避免双写与累计误差。
