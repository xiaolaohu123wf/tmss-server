...
阶段 14 轨迹分段重构   ██████████  ✅ 完成    六类型精确分段 + 段点解耦 + idle 隐藏段
阶段 15 大屏与设备一致性 ██████████ ✅ 完成    日期筛选联动 + 轨迹高亮纠偏 + IMEI 恢复语义
```
...
- [x] 大屏新增统一日期筛选组件（`DateRangePicker`），左中右面板统计按同一时间窗查询
- [x] 地图点击车辆轨迹高亮修正：优先匹配实时在途段，其次开放段，最后兜底最新段
- [x] 轨迹点查询补齐 `buffer_min` 透传，运输段前后缓冲展示与后端一致
- [x] 设备新增语义统一：同 IMEI 软删除记录优先恢复；活跃重复 IMEI 返回 `409 conflict`
...
**结果**：已完成轨迹分段重构，实现六类型精确分段，原始定位数据不变，段与点解耦，idle 停车段默认隐藏。

> 设计规范详见 `ARCHITECTURE.md §附录E`，数据库变更详见 `DATABASE.md §V007`。
> **依赖**：阶段 6（业务逻辑）+ 阶段 11（轨迹查询）
> 下方清单保留为实施留痕（历史记录），当前以代码与发布记录为准。
